...
from utils.Conformer import Conformer
...
if __name__=="__main__":
    ...
    parser.add_argument('--eeg_dataset',
                        type=str,
                        default="./data/eeg/theperils/spampinato-1-IMAGE_RAPID_5Hz_96Hz.pth",
    ...
    # LSTM_model = Model(input_size=96,lstm_size=384,lstm_layers=4,output_size=features_length, include_top=False)
    # LSTM_model.to(device)

    Conformer_model = Conformer(emb_size=40,depth=6,n_classes=features_length)
    Conformer_model.to(device)



    tok, outputs = Conformer_model(eeg.to(device))
    print("Token ", tok.size(), outputs.size())

    opt = torch.optim.Adam(lr=learning_rate, params=Conformer_model.parameters())
    ...
    for EPOCH in range(EPOCHS):
        ...
        if EPOCH % validation_frequency==0 and EPOCH>0:
            ...
            gallery_features, gallery_labels = dataset.transformEEGDataLSTMByList(model=Conformer_model,
                                                                                  data_loader=data_loader_train)
            query_features, query_labels = dataset.transformEEGDataLSTMByList(model=Conformer_model,
            ...
            if best_val_loss is None:
                ...
                torch.save(Conformer_model.state_dict(), f"{FLAGS.log_dir}/conformer_dinov2_best_loss.pth")
            else:
